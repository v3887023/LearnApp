...
class CropImageView @JvmOverloads constructor(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (Style.RECTANGLE == mStyle) {
            ...
            val radius = Math.min(
                (mFocusRect.right - mFocusRect.left) / 2,
        ...
        if (Style.RECTANGLE == mStyle) {
            //宽度自定义调节
            val w = mFocusRect.width() / 3
            val h = mFocusRect.height() / 3
            canvas.drawLine(
                mFocusRect.left,
                h + mFocusRect.top,
                mFocusRect.right,
...
//                    bitmap.recycle();
    ...
    fun saveBitmapToFile(
        ...
        var outputFormat = CompressFormat.JPEG
...
//        }
     ...
     * 将图片保存在本地
     */
    private fun saveOutput(croppedImage: Bitmap?, outputFormat: CompressFormat, saveFile: File) {
    ...
    init {
        mFocusWidth = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_DIP,
            mFocusWidth.toFloat(),
            resources.displayMetrics
        ).toInt()
        mFocusHeight = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_DIP,
            mFocusHeight.toFloat(),
            resources.displayMetrics
        ).toInt()
        ...
        mBorderWidth =
            a.getDimensionPixelSize(R.styleable.CropImageView_cropBorderWidth, mBorderWidth)
        mFocusWidth = a.getDimensionPixelSize(R.styleable.CropImageView_cropFocusWidth, mFocusWidth)
        mFocusHeight =
            a.getDimensionPixelSize(R.styleable.CropImageView_cropFocusHeight, mFocusHeight)
